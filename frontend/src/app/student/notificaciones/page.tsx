...
const typeIcon: Record<string, { icon: string; color: string }> = {
  ...
  FOLLOW_REQUEST:     { icon: 'person_add',    color: 'text-primary' },
  FOLLOW_ACCEPTED:    { icon: 'how_to_reg',    color: 'text-green-500' },
  NEW_FOLLOWER:       { icon: 'person_add',    color: 'text-primary' },
...
export default function NotificacionesPage() {
  ...
  const remove = async (id: string) => {
    ...
    api.delete(`/notifications/${id}`).catch(() => {
      // If delete fails, reload to restore actual state
      load()
    })
  }
